...
*/	
	...
	function services( $atts ) {
		...
		$s .= '</div>';
		$s .= '</div>';
		$href = add_query_arg( array( "app_service_id" => "'+selected_service", "app_provider_id" => false ) );
	...
	function get_excerpt( $page_id, $thumb_size, $thumb_class, $worker_id=0 ) {
		...
		$text = apply_filters('app_the_content', $text, $page_id );
	...
	function get_content( $page_id, $thumb_size, $thumb_class, $worker_id=0 ) {
		...
		return apply_filters('app_the_content', $thumb. wpautop( $page->post_content ), $page_id);
	}
	...
	function post_confirmation() {
		...
		// Save previous status to check if we forced it
		...
		if ( $previous_status != $status && 'confirmed' == $status && isset( $this->options["send_confirmation"] ) 
			&& 'yes' == $this->options["send_confirmation"] )
	...
	function is_holiday( $ccs, $cce ) {
		...
		return apply_filters( 'app_is_exceptional_holiday', $is_holiday, $ccs, $cce, $this->service, $this->worker );
	}
	...
	function is_busy( $start, $end, $capacity ) {
		...
		// Then we must look for anon workers apps too. There are 2 cases here:
		// 1) There are several providers: Look for reserve apps for for this service.
...
*/
	...
	function save_profile( $profileuser_id ) {
		...
		if ( ( $current_user->ID != $profileuser_id || !$this->is_worker($profileuser_id ) ) )
			return;
	...
	?>
		...
		<td>
		<?php echo do_shortcode("[app_my_appointments provider_id=".$profileuser->ID."  provider=1 ".$gcal."]") ?>
		</td>
		</tr>
...
********************************
	...
	function send_confirmation( $app_id ) {
		...
		if ( $r != null ) {
			...
			$mail_result = wp_mail( 
						$r->email,
						$this->_replace( $this->options["confirmation_subject"], $r->name, $this->get_service_name( $r->service), $this->get_worker_name( $r->worker), $r->start, $r->price, $this->get_deposit($r->price) ),
						$this->_replace( $this->options["confirmation_message"], $r->name, $this->get_service_name( $r->service), $this->get_worker_name( $r->worker), $r->start, $r->price, $this->get_deposit($r->price) ),
						$this->message_headers( true, $worker_email )
					);
			if ( $r->email && $mail_result && isset( $this->options["log_emails"] ) && 'yes' == $this->options["log_emails"] )
			...
			if ( $results ) {
				foreach ( $results as $r ) {
					...
					$messages[] = array(
								'ID'		=> $r->ID,
								'to'		=> $r->email,
								'subject'	=> $this->_replace( $this->options["reminder_subject"], $r->name, $this->get_service_name( $r->service), $this->get_worker_name( $r->worker), $r->start, $r->price, $this->get_deposit($r->price) ),
								'message'	=> $this->_replace( $this->options["reminder_message"], $r->name, $this->get_service_name( $r->service), $this->get_worker_name( $r->worker), $r->start, $r->price, $this->get_deposit($r->price) )
							);
					// Update "sent" field		
			...
			if ( $results ) {
				foreach ( $results as $r ) {
					...
					$messages[] = array(
								...
								'subject'	=> $this->_replace( $this->options["reminder_subject"], $r->name, $this->get_service_name($r->service), $this->get_worker_name($r->worker), $r->start, $r->price, $this->get_deposit($r->price) ),
								'message'	=> $provider_add_text . $this->_replace( $this->options["reminder_message"], $r->name, $this->get_service_name( $r->service), $this->get_worker_name( $r->worker), $r->start, $r->price, $this->get_deposit($r->price) )
							);
					// Update "sent" field		
					$wpdb->update( $this->app_table,
	 ...
	 */	
	function _replace( $text, $user, $service, $worker, $datetime, $price, $deposit ) {
		return str_replace( 
					array( "SITE_NAME", "CLIENT", "SERVICE_PROVIDER", "SERVICE", "DATE_TIME", "PRICE", "DEPOSIT" ),
					array( get_option( 'blogname' ), $user, $worker, $service, mysql2date( $this->datetime_format, $datetime ), $price, $deposit ),
					$text
				);
	}
	...
	?>
		...
		<div class="postbox">
            ...
            <div class="inside">
				...
				<tr valign="top">
					...
					<span class="description"><?php _e('You may want to receive a notification email whenever a new appointment is taken from front end in pending status. This email is only sent if you do not require a payment, that is, if your approval is required.', 'appointments') ?></span>
					</td>
					</tr>
   				<tr>
				...
				<td>
				<span class="description">
				<?php _e('For the above email subject and message contents, you can use the following placeholders which will be replaced by their real values:', 'appointments') ?>&nbsp;SITE_NAME, CLIENT, SERVICE, SERVICE_PROVIDER, DATE_TIME, PRICE, DEPOSIT
				</span>
				</td>
				</tr>
		...
		<br />
			...
			<li><?php _e('<b>Parameters:</b>', 'appointments' ) ?>
				...
				<li><?php _e('autorefresh: If set as 1, Show button will not be displayed and page will be automatically refreshed as client picks a new selection. Note: Client cannot browse through the selections and thus check descriptions on the fly (without the page is refreshed). Default: "0" (disabled)', 'appointments' ) ?></li>
				</ul>
			</li>
			</ul>
		<br />
