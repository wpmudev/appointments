...
*/	
	...
	function services( $atts ) {
		...
		$s .= '</div>';
		$s .= '</div>';
		$href = apply_filters( 'app_service_href', 
				add_query_arg( array( "app_service_id" => "'+selected_service", "app_provider_id" => false, "wcalendar" => false ) ) );
	...
	function service_providers( $atts ) {
		...
		$s .= '</div>';
		$href = apply_filters( 'app_worker_href',
				add_query_arg( array( "app_provider_id" => "'+selected_worker", "wcalendar" => false ) ) );
				...
				foreach( $days as $day_name=>$day ) {
					if ( $day_name == $this_days_name && isset( $day["active"] ) && 'yes' == $day["active"] ) {
						...
						if ( $ccs >= strtotime( $this_day. " ". $this->to_military( $day["start"] ), $this->local_time ) &&
							$cce <= strtotime( $this_day. " ". $end, $this->local_time ) )
							return true;
					}
				}
			}
...
********************************
	...
	function _replace( $text, $user, $service, $worker, $datetime, $price, $deposit, $phone, $note, $address, $email ) {
		return str_replace( 
					array( "SITE_NAME", "CLIENT", "SERVICE_PROVIDER", "SERVICE", "DATE_TIME", "PRICE", "DEPOSIT", "PHONE", "NOTE", "ADDRESS", "CLIENT_EMAIL" ),
					array( get_option( 'blogname' ), $user, $worker, $service, mysql2date( $this->datetime_format, $datetime ), $price, $deposit, $phone, $note, $address, $email ),
					$text
				);
	}
...
*/
	...
	function admin_init() {
		...
		if ( isset($_POST["action_app"]) && 'save_general' == $_POST["action_app"] ) {
			...
			$this->options["gcal_same_window"]			= isset( $_POST["gcal_same_window"] );
	...
	?>
		...
		<div class="postbox">
            ...
            <div class="inside">
				...
				<td>
				<span class="description">
				<?php _e('For the above email subject and message contents, you can use the following placeholders which will be replaced by their real values:', 'appointments') ?>&nbsp;SITE_NAME, CLIENT, SERVICE, SERVICE_PROVIDER, DATE_TIME, PRICE, DEPOSIT, PHONE, NOTE, ADDRESS, CLIENT_EMAIL
				</span>
				</td>
				</tr>
		...
		<ul id='q37'>
			<li>
			<?php _e('<b>I am running a website where I am the only service provider. I want to accept more than one appointment for each time slot. Entering higher numbers in "capacity" field in Services tab does work. Why?</b>', 'appointments');?>
			<br />
			...
			<br />&nbsp;&nbsp;&nbsp;&nbsp;return 10;
			<br />}
			<br />add_filter( 'app_schedule_starting_hour', 'increase_capacity', 10, 3 );
			</code>
			</li>
		</ul>
		...
		<ul id='q40'>
			...
			<br />&nbsp;&nbsp;&nbsp;&nbsp;return str_replace("window.location.href=window.location.href", "window.location.href='http://example.com'", $script);
			<br />}
