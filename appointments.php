...
********************************
	...
	function _replace( $text, $user, $service, $worker, $datetime ) {
		return str_replace( 
					...
					array( get_option( 'blogname' ), $user, $worker, $service, mysql2date( $this->datetime_format, $datetime ) ),
					$text
				);
	}
...
';
	...
	function inline_edit() {
		...
		if ( $app_id ) 
			...
			if ( $this->locale_error ) {
				$app->start = date( $this->datetime_format, $this->local_time + 60*$this->min_time );
				$app->end = date( $this->datetime_format, $this->local_time + 120*$this->min_time );
			}
			else {
		...
		$html .= '<span class="input-text-wrap" >';
		if ( $this->locale_error )
			$start = date( $this->date_format, strtotime($app->start) );
		else
			$start = date_i18n( $this->date_format, strtotime($app->start) );
		$html .= '<input type="text" name="date" class="datepicker" size="12" value="'.$start.'" />';
		$html .= '</label>';
		...
		for ( $t=0; $t<3600*24; $t=$t+$min_secs ) {
			$dhours = $this->secs2hours( $t ); // Hours in 08:30 format
			if ( $dhours == date_i18n( $this->time_format, strtotime($app->start) ) )
				$s = " selected='selected'";
			else $s = '';
		...
		if ( $app_id ) {
			...
			if ( $this->locale_error )
				...
				$html .= date_i18n( $this->datetime_format, strtotime($app->end) );
			$html .= '</span>';
			$html .= '</label>';
		}
		/* Note */
	...
	function inline_edit_save() {
		...
		$data['start']		= date( 'Y-m-d H:i:s', strtotime( $this->to_us( $_POST['date'] ). " " . $_POST['time'] ) );
		$data['end']		= date( 'Y-m-d H:i:s', strtotime( $this->to_us( $_POST['date'] ). " " . $_POST['time'] ) + $service->duration *60 );
