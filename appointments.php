...
********************************
				...
				foreach ( $results as $r ) {
					...
					$messages[] = array(
								...
								'message'	=> apply_filters( 'app_reminder_message', $this->add_cancel_link( $this->_replace( $this->options["reminder_message"], 
									...
									$r->price, $this->get_deposit($r->price), $r->phone, $r->note, $r->address, $r->email, $r->city ), $r, $app_id ), $app_id )
							);
					// Update "sent" field		
	...
	function cancel() {
		...
		if ( isset( $this->options['allow_cancel'] ) && 'yes' == $this->options['allow_cancel'] ) {
			...
			if ( isset( $_GET['app_cancel'] ) && isset( $_GET['app_id'] ) && isset( $_GET['app_nonce'] ) ) {
				...
				// Also the clicked link may belong to a formerly created and deleted appointment. Another irrelevant app may have been created. So we will check creation date
	...
	function add_cancel_link( $text, $app_id ) { 
		if ( isset( $this->options['allow_cancel'] ) && 'yes' == $this->options['allow_cancel'] && $app_id ) {
			// The link to be clicked may belong to a formerly created and deleted appointment. Another irrelevant app may have been created. So we will add creation date for check
			$app = $this->get_app( $app_id );
...
';
	...
	?>
		...
		<ul id='q43'>
			...
			<?php _e('Please also note that this is a kind of "hack" and when you have more than one service provider, this function may not work as expected in regards to working hours, as "virtual" providers will not be bound to working hours of existing providers. ', 'appointments');?>
			<br />
		</ul>
