		...
		if ( $r != null ) {
			...
			if ( $r->worker &&  $admin_email != $this->get_worker_email( $r->worker ) && isset( $this->options['allow_worker_confirm'] ) && 'yes' == $this->options['allow_worker_confirm'] ) {
				$mail_result = wp_mail( 
					...
					sprintf( __('The new appointment has an ID %s for %s and you can confirm it using your profile page.','appointments'), $app_id, date_i18n( $this->datetime_format, strtotime($r->start) ) ),
					$this->message_headers( )
				);
			...
			if ( $results ) {
				foreach ( $results as $r ) {
					...
					$messages[] = array(
								...
								'message'	=> apply_filters( 'app_reminder_message', $this->_replace( $this->options["reminder_message"], $r->name, $this->get_service_name( $r->service), $this->get_worker_name( $r->worker), $r->start, $r->price, $this->get_deposit($r->price), $r->phone, $r->note, $r->address, $r->email ), $r, $app_id )
							);
					// Update "sent" field		
