...
********************************
	...
	function send_confirmation( $app_id ) {
		...
		if ( $r != null ) {
			...
			if ( is_object( $worker_data ) && $worker_data && $worker_data->user_email )
...
';
	...
	function safe_date_format() {
		if ( 'F j, Y' == $this->date_format || 'd F Y' == $this->date_format || 'Y/m/d' == $this->date_format 
	...
	/**
	 * Modify a date if it is in European date format
	 * Change d/m/y to d-m-y so that strtotime can behave correctly
	 * @return string
	 * @since 1.0.4.2
	 */	
	function to_us( $date ) {
		$date_arr = explode( '/', $this->date_format );
		if ( isset( $date_arr[0] ) && isset( $date_arr[1] ) && 'd/m' == $date_arr[0] .'/'. $date_arr[1] )
			return str_replace( '/', '-', $date );
		// If not European in date format do nothing	
		return $date;
	}
	
	// TODO: Localize datepick
