...
*/
	 ...
	 * @return array of objects
	 */	
	function get_reserve_apps_by_service( $l, $s ) {
		$apps = wp_cache_get( 'reserve_apps_by_service_'. $l . '_' . $s );
		if ( false === $apps ) {
			$workers = $this->get_workers_by_service( $s );
			$apps = array();
			if ( $workers ) {
				foreach ( $workers as $worker ) {
					$apps_service = $this->get_reserve_apps( $l, $s, $worker->ID );
					if ( $apps_service )
						$apps = array_merge( $apps, $apps_service );
				}
			}
			// Also include appointments by general staff for this service
			$apps_service_0 = $this->get_reserve_apps( $l, $s, 0 );
			if ( $apps_service_0 )
			...
			wp_cache_set( 'reserve_apps_by_service_'. $l . '_' . $s, $apps );
		}
		return $apps;
	}
	...
	function get_worker_email( $worker=0 ) {
		...
		if ( isset( $this->options['dummy_assigned_to'] ) && $this->options['dummy_assigned_to'] ) {
			...
			if ( $worker_data )
				...
				$worker_email = '';
			return apply_filters( 'app_dummy_email', $worker_emaill, $worker );
		}
...
*/	
	...
	function services( $atts ) {
		...
		$s .= '</div>';
		$s .= '</div>';
		$href = add_query_arg( array( "app_service_id" => "'+selected_service", "app_provider_id" => false, "wcalendar" => false ) );
		...
		$script .= "if (typeof selected_service=='undefined'){";
	...
	function service_providers( $atts ) {
		...
		$s .= '</div>';
		$href = add_query_arg( array( "app_provider_id" => "'+selected_worker", "wcalendar" => false ) );
		...
		$script .= "if (typeof selected_worker=='undefined'){";
	...
	function get_content( $page_id, $thumb_size, $thumb_class, $worker_id=0 ) {
		...
		return apply_filters('app_content', $thumb. wpautop( $page->post_content ), $page_id, $worker_id);
	}
	...
	function post_confirmation() {
		...
		if ( $this->check_marketpress_plugin() && 'product' == $post->post_type && strpos( $post->post_content, '[app_' ) !== false ) {
			$mp = 1;
			$variation = $this->add_variation( $wpdb->insert_id, $post_id );
		}
		else
			$mp = $variation = 0;
	...
	function is_exceptional_working_day( $ccs, $cce ) {
		...
		return apply_filters( 'app_is_exceptional_working_day', $is_working_day, $ccs, $cce, $this->service, $this->worker );
	}
	...
	function is_holiday( $ccs, $cce ) {
		...
		return apply_filters( 'app_is_holiday', $is_holiday, $ccs, $cce, $this->service, $this->worker );
	}
				...
				if ( is_array( $days ) ) {
					...
					foreach( $days as $day_name=>$day ) {
						if ( $day_name == $this_days_name && isset( $day["active"] ) && 'yes' == $day["active"] ) {
							...
							if ( $allow_overwork ) {
								if ( $ccs >= strtotime( $this_day . " " . $end_mil , $this->local_time ) )
									return false;
							}
							else {
								if (  $css_plus_duration > strtotime( $this_day . " " . $end_mil , $this->local_time ) ) 
									return false;
							}
	...
	/**
	 * Return available number of workers for a time frame
	...
	function is_busy( $start, $end, $capacity ) {
		...
		// If we are here, no preference is selected (provider_id=0). There are 2 cases here:
		...
		if ( $this->get_workers() != null ) {
			...
			if ( $workers ) {
				foreach( $workers as $worker ) {
					$app_worker = $this->get_reserve_apps_by_worker( $this->location, $worker->ID );
					...
					if ( $services_provided && is_array( $services_provided ) && !empty( $services_provided ) ) {
						foreach ( $services_provided as $service_ID ) {
							$apps_service_0 = $this->get_reserve_apps( $this->location, $service_ID, 0 );
			...
			$apps = $this->get_reserve_apps_by_worker( $this->location, 0 );
...
*/
	...
	function show_profile( $profileuser ) {
		...
		// For other than user himself, doisplay data as readonly
...
*/
	...
	function screen_content_my_app() {
		...
		if ( !$this->is_worker( $user_id ) )
			...
			echo do_shortcode("[app_my_appointments _allow_confirm=1 provider=1 ".$gcal."]");
	}
...
********************************
	...
	function send_confirmation( $app_id ) {
		...
		if ( $r != null ) {
			...
			$body = apply_filters( 'app_confirmation_message', $this->_replace( $this->options["confirmation_message"], $r->name, $this->get_service_name( $r->service), 
					$this->get_worker_name( $r->worker), $r->start, $r->price, $this->get_deposit($r->price), $r->phone, $r->note, $r->address ), $r, $app_id );
			
			$mail_result = wp_mail( 
						$r->email,
						$this->_replace( $this->options["confirmation_subject"], $r->name, $this->get_service_name( $r->service), $this->get_worker_name( $r->worker), $r->start, $r->price, $this->get_deposit($r->price), $r->phone, $r->note, $r->address ),
						$body,
			...
			if ( $r->email && $mail_result ) {
				...
				$provider_add_text  = sprintf( __('A new appointment has been made on %s. The below is a copy of what has been sent to your client:', 'appointments'), get_option( 'blogname' ) );
				$provider_add_text .= "\n\n\n";

				wp_mail( 
						$to,
						$this->_replace( __('New Appointment','appointments'), $r->name, $this->get_service_name( $r->service), $this->get_worker_name( $r->worker), $r->start, $r->price, $this->get_deposit($r->price), $r->phone, $r->note, $r->address ),
			...
			if ( $results ) {
				foreach ( $results as $r ) {
					...
					$messages[] = array(
								'ID'		=> $r->ID,
								'to'		=> $r->email,
								'subject'	=> $this->_replace( $this->options["reminder_subject"], $r->name, $this->get_service_name( $r->service), $this->get_worker_name( $r->worker), $r->start, $r->price, $this->get_deposit($r->price), $r->phone, $r->note, $r->address ),
								'message'	=> $this->_replace( $this->options["reminder_message"], $r->name, $this->get_service_name( $r->service), $this->get_worker_name( $r->worker), $r->start, $r->price, $this->get_deposit($r->price), $r->phone, $r->note, $r->address )
							);
					// Update "sent" field		
			...
			if ( $results ) {
				foreach ( $results as $r ) {
					...
					$messages[] = array(
								...
								'subject'	=> $this->_replace( $this->options["reminder_subject"], $r->name, $this->get_service_name($r->service), $this->get_worker_name($r->worker), $r->start, $r->price, $this->get_deposit($r->price), $r->phone, $r->note, $r->address ),
								'message'	=> $provider_add_text . $this->_replace( $this->options["reminder_message"], $r->name, $this->get_service_name( $r->service), $this->get_worker_name( $r->worker), $r->start, $r->price, $this->get_deposit($r->price), $r->phone, $r->note, $r->address )
							);
					// Update "sent" field		
					$wpdb->update( $this->app_table,
	 ...
	 */	
	function _replace( $text, $user, $service, $worker, $datetime, $price, $deposit, $phone, $note, $address ) {
		return str_replace( 
					array( "SITE_NAME", "CLIENT", "SERVICE_PROVIDER", "SERVICE", "DATE_TIME", "PRICE", "DEPOSIT", "PHONE", "NOTE", "ADDRESS" ),
					array( get_option( 'blogname' ), $user, $worker, $service, mysql2date( $this->datetime_format, $datetime ), $price, $deposit, $phone, $note, $address ),
					$text
				);
	}
...
';
	...
	?>
			...
			<form method="post" action="" >
				...
				<div class="postbox">
					...
					<div class="inside">
						...
						<span class="description"><?php _e('If the above time bases do not fit your business, you can add a new one, e.g. 240. Only numbers divisible by 10 or 15 are accepted. Note: After you save this additional time base, you must select it using the above setting.', 'appointments') ?></span>
						</tr>
		...
		<div class="postbox">
            ...
            <div class="inside">
				...
				<td>
				<span class="description">
				<?php _e('For the above email subject and message contents, you can use the following placeholders which will be replaced by their real values:', 'appointments') ?>&nbsp;SITE_NAME, CLIENT, SERVICE, SERVICE_PROVIDER, DATE_TIME, PRICE, DEPOSIT, PHONE, NOTE, ADDRESS
				</span>
				</td>
				</tr>
		...
		<?php case 'services': _e( '<i>Here you should define your services for which your client will be making appointments. <b>There must be at least one service defined.</b> Capacity is the number of customers that can take the service at the same time. Enter 0 for no specific limit (Limited to number of service providers, or to 1 if no service provider is defined for that service). Price is only required if you request payment to accept appointments. You can define a description page for the service you are providing.</i>', 'appointments') ?>
			...
			<form method="post" action="" >
				...
				<tr><td colspan="5"><span class="description" style="font-size:10px"><?php _e('* <b>You must select at least one service, otherwise provider will not be saved!</b> Use CTRL and SHIFT keys to select multiple options', 'appointments') ?></span>
				</td>
				</tr>
				<?php
				$workers = $wpdb->get_results("SELECT * FROM " . $this->workers_table . " " ); 
				...
				&nbsp;&nbsp;
				<?php _e( '<i>Tip: To remove a service provider, by the help of CTRL and SHIFT keys clear out "Services Provided" selections and save.</i>', 'appointments' ); ?>
				</div>
		...
		<ul id='q3'>
			...
			<br />
			<?php _e('You can add one more time base using <i>Additional time base</i> setting. This value must be divisible by 10 or 15. Otherwise it will not be included in the selectable time bases. You must select this setting in <i>time base</i> setting to be effective.', 'appointments');?>
			</li>
		</ul>
		...
		<ul id='q10'>
			...
			<br />
			<?php printf( __('Yes. Use "add" parameter of schedule shortcode to add additional schedules. There is no limit for the number of schedules that you can use on the same page. See %s tabs for details.', 'appointments'), '<a href="'.admin_url('admin.php?page=app_settings&tab=shortcodes').'">'.__('Shortcodes', 'appointments') .'</a>');?>
			</li>
		</ul>
		<ul id='q11'>
			<li>
		...
		<ul id='q22'>
			...
			<br />
			<?php _e('Yes. Create a "dummy" service provider and assign that particular service only to it. Then set its working days as those days. That\'s all.', 'appointments');?>
			</li>
		</ul>
		...
		<ul id='q36'>
			...
			<br />
			<?php _e('Yes. Referring this filter create a function in your functions.php to set status as "confirmed" for the selected user, user role and/or service: <code>$status = apply_filters( \'app_post_confirmation_status\', $status, $price, $service, $worker, $user_id );</code>. If you confirm your appointments with this method, confirmation email will be sent automatically, unless you did not disable them. ', 'appointments');?>
			</li>
		</ul>
	...
	function inline_edit() {
		...
		if ( $app_id ) {
			...
			if ( $this->locale_error )
				$start_date = date( $this->date_format, strtotime( $app->start ) );
			else
				$start_date = date_i18n( $this->date_format, strtotime( $app->start ) );
			...
			if ( $this->locale_error ) {
				$start_date = date( $this->date_format, $this->local_time + 60*$this->min_time );
			}
			else {
				$start_date = date_i18n( $this->date_format, $this->local_time + 60*$this->min_time );
			}
		}
