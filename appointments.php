...
*/
	...
	/**
	 * Check if this is a product page and if it is, modify it
	 * @ Since 1.0.1 
	 */	
...
*/
	...
	function load_scripts_styles( ) {
		...
		$ajaxurl_added = false;
		if ( !@$this->options["disable_js_check_frontend"] && current_user_can('manage_options') ) {
		...
		if ($this->options['accept_api_logins']) {
			...
			if ( !$ajaxurl_added )
				wp_localize_script( 'appointments_api_js', '_appointments_data',
					array(
						'ajax_url' => admin_url('admin-ajax.php'),
						'root_url' => plugins_url('appointments/images/')
						)
			...
			<tr class="payment_row" <?php if ( $this->options['payment_required'] != 'yes' ) echo 'style="display:none"'?>>
	          ...
	          ?>
				...
				<tr class="payment_row" <?php if ( $this->options['payment_required'] != 'yes' ) echo 'style="display:none"'?>>
					...
					<br />
					<?php
					printf( __('Just for your information, your IPN link is %s. Knowing this may be useful in some cases.', 'appointments'), admin_url('admin-ajax.php?action=app_paypal_ipn') );
					?>
					</td>
				</tr>
