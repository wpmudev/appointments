...
';
	...
	function to_us( $date ) {
		...
		if ( strpos( $date_format, 'M' ) !== false ) {
			// Check if M (long month name) is set
			$month_abb = array( );
			// Generate an abbr array like January=>Oca
			foreach ( $months as $month_name => $month_no ) {
				$month_name_local = $wp_locale->get_month($month_no);
	...
	function inline_edit() {
		...
		$html .= '<span class="input-text-wrap" >';
		$html .= '<input type="text" name="date" class="datepicker" size="12" value="'.date_i18n( $this->date_format, strtotime($app->start) ).'" />';
		$html .= '</label>';
