...
*/	
	...
	function service_providers( $atts ) {
		...
		if ( 0 == $this->service )
			...
			$workers = $this->get_workers_by_service( $this->service, $order_by );
	...
	function get_content( $page_id, $thumb_size, $thumb_class, $worker_id=0 ) {
		...
		return apply_filters('app_content', $thumb. wpautop( $this->strip_app_shortcodes( $page->post_content ) ), $page_id, $worker_id);
	}
...
*/
	...
	?>
		...
		<?php if ( isset($this->options["gcal_api_allow_worker"]) && 'yes' == $this->options["gcal_api_allow_worker"] ) { ?>
			...
			<tr>
			<td colspan="2">
			<?php $this->gcal_api->display_nag( $profileuser->ID ); ?>
		</td></tr>
		<?php
...
*/
	...
	function admin_notices() {
		...
		if ( !$this->db_version || version_compare( $this->db_version, '1.2.0', '<' ) ) {
			echo '<div class="error"><p>' .
...
';
	...
	function appointment_list() {
		...
		<li><?php _e('<b>Reserved by GCal:</b> If you import appointments from Google Calender using Google Calendar API, that is, synchronize your calendar with Appointments+, events in your Google Calendar will be regarded as appointments and they will be shown here', 'appointments') ?></li>
			...
			<div class="alignleft actions">
				<form method="get" action="<?php echo add_query_arg('page', 'appointments'); ?>" >
					...
					<select name="app_order_by" style='float:none;'>
						...
						<option value="start_DESC" <?php selected( $order_by, 'start_DESC' ); ?>><?php _e('Appointment date (Newest to oldest)','appointments'); ?></option>
						<option value="start" <?php selected( $order_by, 'start' ); ?>><?php _e('Appointment date (Oldest to newest)','appointments'); ?></option>
					</select>			
	...
	function inline_edit_save() {
		...
		if( $app != null ) {
			...
			if ( $update_result ) {
				if ( 'pending' == $data['status'] || 'removed' == $data['status'] || 'completed' == $data['status'] )
					$this->gcal_api->delete( $app_id );
				else if ( 'paid' == $data['status'] || 'confirmed' == $data['status'] )
					$this->gcal_api->update( $app_id );
			}
