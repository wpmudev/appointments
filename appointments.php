...
*/	
	...
	/**
	 * Shortcode showing all appointments
	 * A simplified, but useful version of my_appointments method
	 * @since 1.2.7
	 */
	function all_appointments( $atts ) {
	
		extract( shortcode_atts( array(
		'title'			=> __('<h3>All Appointments</h3>', 'appointments' ),
		...
		$results = $wpdb->get_results("SELECT * FROM " . $this->app_table . " WHERE (".$stat.") ORDER BY ".$order_by." " );
		...
		$ret .= apply_filters( 'app_all_appointments_column_name', 
			...
			. '</th><th class="all-appointments-worker">' . __('Client', 'appointments' ) 
		...
		$ret .= '</tbody></table>';
		$ret  = apply_filters( 'app_all_appointments_after_table', $ret );
	...
	function my_appointments( $atts ) {
		...
		if ( $allow_cancel && !$provider && isset( $this->options['allow_cancel'] ) && 'yes' == $this->options['allow_cancel'] )
			$a_cancel = true;
		else
			$a_cancel = false;
		...
		$ret .= apply_filters( 'app_my_appointments_column_name', '<th class="my-appointments-service">'. __('Service', 'appointments' ) . '</th><th class="my-appointments-worker">' . $provider_or_client . 
			'</th><th class="my-appointments-date">' . __('Date/time', 'appointments' ) . '</th><th class="my-appointments-status">' 
	...
	function pagination( $atts ) {
		...
		if ( $date && !isset( $_GET["wcalendar"] ) ) {
			$time = strtotime( $date, $this->local_time );
			$_GET["wcalendar"] = $time;
		}
		else {	
			if ( isset( $_GET["wcalendar"] ) )
		...
		if ( !$month ) {
			...
			$next_max = $this->local_time + ($this->get_app_limit() + 7*$step ) *86400;
			...
			$next_max = $this->first_of_month( $this->local_time, $step ) + $this->get_app_limit() * 86400;
	...
	function monthly_calendar( $atts ) {
		...
		// Force a date
		if ( $date && !isset( $_GET["wcalendar"] ) ) {
			$time = $this->first_of_month( strtotime( $date, $this->local_time ), $add );
			$_GET["wcalendar"] = $time;
		}
		else {
			if ( isset( $_GET["wcalendar"] ) )
	...
	function weekly_calendar( $atts ) {
		...
		// Force a date
		if ( $date && !isset( $_GET["wcalendar"] ) ) {
			$time = strtotime( $date, $this->local_time ) + ($add * 7 * 86400) ;
			$_GET["wcalendar"] = $time;
		}
		else {
			if ( isset( $_GET["wcalendar"] ) )
...
*/
	...
	function mp_buy_button_tag( $button, $product_id, $context ) {
		...
		if ( isset($_REQUEST['order'] ) ) {
			$button = preg_replace( '%<input class="mp_button_buynow"(.*?)value="(.*?)" />%is', 
			'<input class="mp_button_buynow" type="submit" name="buynow" value="'.$book_now.'" />', $button );
			$button = preg_replace( '%<input class="mp_button_addcart"(.*?)value="(.*?)" />%is', 
			'<input class="mp_button_buynow" type="submit" name="buynow" value="'.$book_now.'" />', $button );
			$button = preg_replace( '%<form(.*?)></form>%is',
			'<a class="mp_link_buynow" href="'.get_permalink($product_id).'">'.$book_now.'</a>', $button );
	...
	function modify_email( $body, $order ) {
		...
		// TODO: Add details of the appointment here. If there are more than one appintment, add the first one
		return $body;
	}
	...
	/**
	 * Check if this is a A+ product page and if it is, add js codes to footer to hide some MP fields
	 * @Since 1.0.1 
	 */	
	...
	function cancel() {
		...
		if ( isset( $this->options['allow_cancel'] ) && 'yes' == $this->options['allow_cancel'] ) {
			...
			if ( isset( $_POST['app_id'] ) && isset( $_POST['cancel_nonce'] ) ) {
				...
				if ( !$owner )
					die( json_encode( array('error'=>esc_js(__('There is an issue with this appointment. Please refresh page and try again. If problem persists, please contact website admin.','appointments') ) ) ) );
...
';
	...
	?>
		...
		<br />
			<ul>
			<li><?php _e('<b>Shortcode:</b>', 'appointments' ) ?><code>[app_all_appointments]</code></li>
			<li><?php _e('<b>Description:</b> Inserts a table that displays all upcoming appointments', 'appointments' ) ?>
			</li>
			<li><?php _e('<b>Parameters:</b>', 'appointments' ) ?>
				<ul>
				<li><?php _e('title: Title text. Default: "All Appointments"', 'appointments' ) ?></li>
				<li><?php _e('status: Which status(es) will be included. Possible values: paid, confirmed, completed, pending, removed, reserved or combinations of them separated with comma. Default: "paid,confirmed"', 'appointments' ) ?></li>
		...
		<br />
			...
			<li><?php _e('<b>Parameters:</b>', 'appointments' ) ?>
				...
				<li><?php _e('allow_cancel: Enter 1 if you want to allow cancellation of appointments by the client using this table. "Allow client cancel own appointments" setting must also be set as Yes. Default: "0" (Cancellation is not allowed).', 'appointments' ) ?></li>
				</ul>
			</li>
			</ul>
		<br />
			...
			<li><?php _e('<b>Parameters:</b>', 'appointments' ) ?>
				...
				<li><?php _e('worker: In some cases, you may want to display services which are given only by a certain provider. In that case enter provider ID here. Note: order_by parameter will not work in combination with this one. Default: "0" (all defined services). Note: Multiple selections are not allowed.', 'appointments' ) ?></li>
				</ul>
			</li>
			</ul>
		<br />
			...
			<li><?php _e('<b>Parameters:</b>', 'appointments' ) ?>
				...
				<li><?php _e('service: In some cases, you may want to force to display providers who give only a certain service. In that case enter service ID here. Default: "0" (list is determined by services dropdown). Note: Multiple selections are not allowed.', 'appointments' ) ?></li>
				</ul>
			</li>
			</ul>
		<br />
		<br />	
			...
			<li><?php _e('<b>Parameters:</b>', 'appointments' ) ?>
				...
				<li><?php _e('service: Enter service ID only if you want to force the table display the service with entered ID. Default: "0" (Service is selected by dropdown). Note: Multiple selections are not allowed.', 'appointments' ) ?></li>
				<li><?php _e('worker: Enter service provider ID only if you want to force the table display the service provider with entered ID. Default: "0" (Service provider is selected by dropdown). Note: Multiple selections are not allowed.', 'appointments' ) ?></li>
		...
		<br />
			...
			<li><?php _e('<b>Parameters:</b>', 'appointments' ) ?>
				...
				<li><?php _e('date: This is only required if this shortcode resides above any schedule shortcodes. Otherwise it will follow date settings of the schedule shortcodes. Default: "0" (Current week or month) ', 'appointments' ) ?></li>
				</ul>
			</li>
			</ul>
		<br />
