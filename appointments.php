...
*/	
	...
	function services( $atts ) {
		...
		$s .= '</div>';
		$s .= '</div>';
		$href = apply_filters( 'app_service_href', add_query_arg( array( "app_service_id" => "'+selected_service", "app_provider_id" => false ) ) );
	...
	function service_providers( $atts ) {
		...
		$s .= '</div>';
		$href = apply_filters( 'app_worker_href', add_query_arg( array( "app_provider_id" => "'+selected_worker" ) ) );
...
';
	...
	function inline_edit() {
		...
		if ( $app_id ) {
			...
			$start_time = date_i18n( $safe_date_format, strtotime( $app->start ) );
