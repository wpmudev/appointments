	...
	function display_settings( $worker_id=0 ) {
		...
		<?php if ( 'sync' == $gcal_api_mode && $gcal_service_account && $gcal_key_file && $gcal_selected_calendar ) {
		 ...
		 <tr>
			...
			<br />
			<span class="description"><?php _e('Clicking this link will manually import and update your Events from the selected calendar without waiting for 10 minutes. Note: Maximum 1000 future events that will start until appointment limit setting are imported in the order of their starting time. Past events and all day events are not imported.', 'appointments') ?></span>
			</td>
		</tr>
			<?php  
		}
...
;		
	...
	function import_and_update_events( $worker_id=0 ) {
		...
		// Get only future events and limit them with appointment limit setting and 1000 events
		$events = $this->service->events->listEvents( $this->get_selected_calendar( $worker_id ), 
			apply_filters( 'app_gcal_args',
				...
				'maxResults'	=> apply_filters( 'app_gcal_max_results', 1000 ),
		...
		if ( $events && is_array( $events->getItems()) ) {
			...
			if ( !empty( $values ) ) {
				...
				$result = $wpdb->query( "INSERT INTO " . $this->app_table . " (created,service,worker,status,start,end,note,gcal_ID,gcal_updated) 
					...
					ON DUPLICATE KEY UPDATE start=VALUES(start), end=VALUES(end), note=VALUES(note), gcal_updated=VALUES(gcal_updated)" );
