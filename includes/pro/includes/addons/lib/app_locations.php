...
class App_Locations_DefaultLocation extends App_Locations_Location {
	...
	public function get_id() {
		return $this->get_var( self::KEY_ID ); }
	public function get_address() {
		return $this->get_var( self::KEY_ADDRESS ); }
}
...
class App_Locations_MappedLocation extends App_Locations_Location {
	...
	public function __construct() {
		...
		if ( ! is_admin() ) {
			$this->_maps_codec = new AgmMarkerReplacer;
		}
	}
	...
	public function to_map() {
		...
		if ( ! $map_id ) {
			return false; // Map creation failed
		}
	...
	private function _create_map() {
		...
		if ( $latitude ) {
			$this->set_var( self::KEY_LAT, $latitude );
		}
		if ( $longitude ) {
			$this->set_var( self::KEY_LNG, $longitude );
		}
		do_action( 'app-locations-location-model-item_updated' );
		return $map_id;
	}
