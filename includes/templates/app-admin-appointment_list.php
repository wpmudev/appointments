...
if ( isset( $_GET['s'] ) ) {
	$s = tag_escape( stripslashes( $_GET['s'] ) );
	$filter['s'] = $s;
	$args['s'] = $s;
} else {
	$s = '';
...
if ( isset( $_GET['app_service_id'] ) ) {
	$service_id = intval( $_GET['app_service_id'] );
...
if ( isset( $_GET['app_provider_id'] ) ) {
	$worker_id = intval( $_GET['app_provider_id'] );
...
<p class="search-box">
	...
	<input type="hidden" value="<?php echo esc_attr( $service_id ); ?>" name="app_service_id" />
	<input type="hidden" value="<?php echo esc_attr( $worker_id ); ?>" name="app_provider_id" />
