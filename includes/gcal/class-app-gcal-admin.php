...
class Appointments_Google_Calendar_Admin {
	...
	public function render_tab() {
		...
		if ( empty( $client_id ) || empty( $client_secret ) ) {
			$version = '2.0.3';
			wp_register_script( 'unslider', appointments_plugin_url() . 'assets/vendor/unslider/js/unslider-min.js', array( 'jquery' ), $version, true );
			wp_enqueue_script( 'app-gcal', appointments_plugin_url() . 'admin/js/admin-gcal.js', array( 'unslider' ), false, true );
			wp_enqueue_style( 'unslider', appointments_plugin_url() . 'assets/vendor/unslider/css/unslider.css', array(), $version );
