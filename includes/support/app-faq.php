<?php
$questions = array(
	__( '-- Select a question --', 'appointments' ),
	__( "How can I restart the tutorial?", 'appointments' ),
	__( "What is the importance of Time Base and how should I set it?", 'appointments' ),
	__( "I don't see the time base that I need. For example I need 240 minutes appointments. How can I do that?", 'appointments' ),
	__( "What is the complete process for an appointment?", 'appointments' ),
	__( "Is it necessary to have at least one service?", 'appointments' ),
	__( "Is it necessary to define service providers?", 'appointments' ),
	__( "Is it necessary to use Services and Service Providers shortcodes?", 'appointments' ),
	__( "Does Appointments+ provide some widgets?", 'appointments' ),
	__( "Can I use the shortcodes in any page as I wish?", 'appointments' ),
	__( "Can I have schedules showing more than two weeks or months on the same page?", 'appointments' ),
	__( "Does the client need to be registered to the website to apply for an appointment?", 'appointments' ),
	__( "How are the appointments confirmed?", 'appointments' ),
	__( "How can I manually confirm an appointment?", 'appointments' ),
	__( "Can I enter a manual appointment from admin side?", 'appointments' ),
	__( "I don't want front end appointments, I want to enter them only manually from admin side. What should I do?", 'appointments' ),
	__( "I don't want my break times and holidays to be seen by the clients. How can I do that?", 'appointments' ),
	__( "How can I prevent a second appointment by a client until I confirm his first appointment?", 'appointments' ),
	__( "I have several service providers (workers) and each of them has different working hours, break hours and holidays. Does Appointments+ support this?", 'appointments' ),
	__( "How can I set start day of the week and adjust date and time formats?", 'appointments' ),
	__( "What does service capacity mean? Can you give an example?", 'appointments' ),
	__( "I have defined several services and service providers. For a particular service, there is no provider assigned. What happens?", 'appointments' ),
	__( "I am giving a service only on certain days of the week, different than my normal working days. Is it possible to set this in Appointments+?", 'appointments' ),
	__( "How can I permanently delete appointment records?", 'appointments' ),
	__( "What happens if a client was applying for an appointment but at the same time another client booked the same time slot?", 'appointments' ),
	__( "What does the Built-in Cache do? Can I still use other caching plugins?", 'appointments' ),
	__( "I have just installed Appointments+ and nothing happens as I click a free time slot on the Make an Appointment page. What can be the problem?", 'appointments' ),
	'',
	__( "How is the plugin supposed to work by the way?", 'appointments' ),
	__( "How does integration with Membership work? Are there any special considerations?", 'appointments' ),
	__( "How does integration with MarketPress work? Are there any special considerations?", 'appointments' ),
	__( "What does DUMMY service provider mean? How can I get use of it?", 'appointments' ),
	__( "How can I view my planner as a service provider in calendar view?", 'appointments' ),
	__( "My working hours cover the midnight and exceeds to the other day. For example from 8pm Monday to 2am Tuesday. Is it possible to set this?", 'appointments' ),
	__( "What are the prerequisites to use Google Calendar API?", 'appointments' ),
	__( "Why do I need this Google Calendar API key file anyway? Isn't there any other way?", 'appointments' ),
	__( "But I am using another application which does not need Google Calendar API key file. How does that application work then?", 'appointments' ),
	__( "Google Calendar Integration is not working and/or I am getting some errors. What can be the reasons and how can I solve them?", 'appointments' ),
	__( "How can I let my service providers freely edit appointments?", 'appointments' ),
	__( "How can I let my clients cancel their own appointments?", 'appointments' ),
	__( "Can I create my own page templates?", 'appointments' ),
	__( "I have customized the front.css file. How can I prevent it being overwritten by plugin updates?", 'appointments' ),
	__( "Is it possible not to ask payment or deposit for certain users?", 'appointments' ),
	__( "How can I force the schedules start at a non standard time, for example 9:15?", 'appointments' ),
	__( 'I want to accept more than one appointment applications for each time slot. Entering higher numbers in "capacity" field in Services tab does not work. Why?', 'appointments' ),
	__( "How can I use HTML in emails?", 'appointments' ),
	__( "I have a time base of 10 minutes. I have services up to 480 minutes. How can I achieve this?", 'appointments' ),
	__( "How can I redirect a user after a confirmed appointment to a thank you page when payment is not required?", 'appointments' ),
	__( "How can I show hours instead of minutes in the front end when my services last more than an hour?", 'appointments' )
);
?>
